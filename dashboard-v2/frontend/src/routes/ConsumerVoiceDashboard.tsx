...
type FilterState = { venture: string; brand: string; productIds: string; dateFrom: string; dateTo: string };
const emptyFilter = (): FilterState => ({ venture: "", brand: "", productIds: "", dateFrom: "", dateTo: "" });
...
function buildParams(filter: FilterState, section: string, extras?: Record<string, string>) {
  ...
  if (filter.productIds) filter.productIds.split(",").forEach((id) => params.append("productId", id));
...
function LocalFilters({ label, value, onChange, options }: {
  ...
  const availableProducts = options?.products.filter((product) => !value.venture || product.venture === value.venture) ?? [];
  const brands = value.venture
    ? [...new Set(availableProducts.map((product) => product.brand))].sort()
    : options?.brands ?? [];
  const products = availableProducts.filter((product) => !value.brand || product.brand === value.brand);
  ...
  return <details className="module-filter">
    ...
    <div className="module-filter-popover" aria-label={`${label} filters`}>
      <label>Venture<select value={value.venture} onChange={(event) => onChange({ ...value, venture: event.target.value, brand: "", productIds: "" })}><option value="">All ventures</option>{options?.ventures.map((item) => <option key={item}>{item}</option>)}</select></label>
      <label>Brand<select value={value.brand} onChange={(event) => onChange({ ...value, brand: event.target.value, productIds: "" })}><option value="">All brands</option>{brands.map((item) => <option key={item}>{item}</option>)}</select></label>
      <label className="product-filter">Product<select value={value.productIds} onChange={(event) => onChange({ ...value, productIds: event.target.value })}><option value="">All products</option>{products.map((item) => <option key={`${item.venture}-${item.id}`} value={item.ids.join(",")}>{item.name}</option>)}</select></label>
  ...
  return <div className="consumer-page compact-consumer-page">
      ...
      {overview.isLoading ? <div className="module-loading">Loading overview…</div> : <>
        ...
        <div className="analytics-grid compact-analytics">
          <article className="panel radar-panel"><header><div><span>Experience dimensions</span><h3>Product, seller and logistics</h3></div><span className="panel-note">/ 5</span></header>
            <div className="dimension-scores">{overviewData?.dimensions?.map((item) => <div key={item.dimension}><span>{item.dimension}</span><strong>{item.score?.toFixed(2) ?? "—"}</strong></div>)}</div>
    ...
    <section className="dashboard-module signals-module">
      ...
      {signals.isLoading ? <div className="module-loading">Loading signals…</div> : <div className="signals-grid compact-signals">
        <article className="panel tag-panel"><header><div><span>Customer language</span><h3>Detailed review themes</h3></div><span className="panel-note">Aspect · sentiment</span></header>
          <div className="tag-cloud">{signalsData?.tags?.length ? signalsData.tags.slice(0, 14).map((tag) => <span key={`${tag.label}-${tag.sentiment}`} className={`tag tag-${tag.sentiment}`} style={{ fontSize: `${Math.min(15, 11 + tag.count * 0.22)}px` }}><span>{tag.label}</span><em>{tag.sentiment}</em><b>{tag.count}</b></span>) : <p className="empty">No recurring themes.</p>}</div>
        </article>
